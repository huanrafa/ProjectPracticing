...
total AS (
  SELECT
    date,
    page_name,
    value AS total_reach,
    post_id AS total_post_id
  FROM source
  WHERE metric_name = 'post_impressions_unique' 
),
organic AS (
  SELECT
    date,
...
reach_per_post AS (
  ...
  FULL JOIN total
  ON organic.date = total.date
    AND organic.page_name = total.page_name
  WHERE organic_post_id = paid_post_id
    AND organic_post_id = total_post_id
  GROUP BY organic.date, organic.page_name, organic_post_id, paid_post_id, total_post_id
)

SELECT
  date,
  page_name,
  SUM(total_reach) AS total_total_reach,
